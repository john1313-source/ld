...
def build_architecture(M):
    ...
    ext = [
        ...
        ("외벽_북측_서재발코니", (18.5, 24.5), (145, 24.5), [W(114.5, 1.9, sill=0.9, head=2.1)]),
        ("외벽_북측_실외기1", (145, 5.5), (145, 24.5), []),
        ("외벽_북측_주방", (145, 5.5), (325, 5.5),
         [W(211.4, 1.5, sill=1.0, head=2.1), W(268.4, 1.0, sill=1.0, head=2.1, split=1)]),
        ...
        ("외벽_남측_자녀방", (263.5, 332.5), (424.5, 332.5),
         [W(299.6, 1.95), W(380, 2.2)]),
        ("외벽_남측_단차", (263.5, 332.5), (263.5, 343.5), []),
        ("외벽_남측_거실안방", (18.5, 343.5), (263.5, 343.5),
         [W(87.2, 2.8), W(201.2, 3.9, sill=0.3, head=2.2, split=3)]),
    ]
    for n, a, b, o in ext:
        wall(n, a, b, T_EXT, M, C, o)

    # ── 비확장 유지 공간 (실외기실 — 대피공간 없음, 사용자 확인) 경계벽 ─────────────────
    keep = [
        ...
        ("유지_실외기D_북", (250, 53.5), (325, 53.5), [D(286, 0.55, head=1.9, swing=1, style="louver")]),
        ("유지_실외기D_칸막이", (296, 53.5), (296, 76.5), []),
        ("유지_실외기D_남", (276.5, 76.5), (325, 76.5), []),
    ]
    for n, a, b, o in keep:
    ...
    inner = [
        ...
        ("벽_서재발코니_주방확장", (162.5, 24.5), (162.5, 53.5), []),
        ("벽_서재_발코니창", (77.5, 53.5), (162.5, 53.5), [W(108.6, 2.1, sill=0.1, head=2.1)]),
        ...
        ("벽_주방_다용도실", (250, 5.5), (250, 53.5), [D(38, 0.8, swing=1, hinge="s", style="balcony")]),
        ...
        if style == "pocket":
            ...
            if horiz:
                ...
                box(f"문_{i}_슬라이딩", f - th / 2, s, 0, f + th / 2, s + (e - s) * 0.3,
                    d["head"], M["frosted"], C)
            continue
        m_leaf = {"entry": M["frame"], "louver": M["louver"],
                  "balcony": M["frosted"]}.get(style, M["paint"])
        w = e - s
...
def build_floors_ceiling(M):
    ...
    floors = [
        ...
        ("바닥_주방", P(162.5, 5.5, 250, 141.5), M["porcelain"], 0.0),
        ("바닥_주방_우측", P(250, 53.5, 276.5, 141.5), M["porcelain"], 0.0),
        ...
        ("바닥_서재뒤발코니", P(48, 24.5, 162.5, 53.5), M["utility"], -0.05),
        ...
        ("바닥_다용도실", P(250, 5.5, 305, 53.5), M["utility"], -0.05),
        # 현관 (단차)
        ...
        ("바닥_실외기D", P(276.5, 53.5, 325, 76.5), M["outdoor_unit"], -0.05),
    ]
...
def build_kitchen(M):
    ...
    kx2 = X(250) - T_INT / 2        # 다용도실 벽 내측 (창측 구간)
    ...
    # 분전함 (업체 설계도 15쪽: 주방 서측 벽 상부장 내부) — 점검용 도어 위치 표시
    box("설비_분전함_점검도어", kx1 + 0.651, 6.75, 1.65, kx1 + 0.656, 7.25, 2.15, M["cab_greige"], C)
    ...
    # 식탁: 아일랜드와 1.15 m 이상 띄우고, 복도 통로(약 1.2 m)는 남쪽에 확보
    tx1, tx2, ty1, ty2 = 5.85, 7.85, 5.3, 6.2
...
def build_dress_bath1(M):
    ...
    # 반신욕조 750(W) (업체 설계도 21쪽) — 북측 벽, 나머지는 젠다이
    tl = min(1.5, bx2 - bx1 - 0.02)
    box("안방욕실_반신욕조", bx1 + 0.01, by2 - 0.75, 0, bx1 + 0.01 + tl, by2 - 0.01, 0.55,
        M["porcelain_ware"], C2, bevel=0.02)
    box("안방욕실_욕조내부", bx1 + 0.1, by2 - 0.66, 0.2, bx1 + tl - 0.08, by2 - 0.1, 0.56,
        M["bath_wall"], C2, bevel=0.04)
    box("안방욕실_젠다이", bx1 + 0.01 + tl, by2 - 0.75, 0, bx2 - 0.01, by2 - 0.01, 0.55,
        M["counter"], C2)
    box("안방욕실_욕조유리", bx1 + 0.01 + tl - 0.7, by2 - 0.76, 0.55, bx1 + 0.01 + tl, by2 - 0.75, 1.9,
        M["glass"], C2)
    box("안방욕실_샤워수전", bx1 + 0.01, by2 - 0.45, 1.0, bx1 + 0.06, by2 - 0.3, 1.15, M["black"], C2)
    box("안방욕실_샤워바", bx1 + 0.01, by2 - 0.39, 1.1, bx1 + 0.03, by2 - 0.37, 1.95, M["black"], C2)
    # 세면대 (동측 벽)
...
def build_study(M):
    ...
    y1 = Y(141.5) + T_INT / 2
    y2 = Y(53.5) - T_INT / 2
    # 서측 벽(안방욕실 벽) 2인 데스크 — 발코니 창은 출입용이라 막지 않음
    desk("서재_2인데스크", x1, y1 + 0.55, x1 + 0.62, y1 + 2.45, M, C)
    chair("서재_의자1", x1 + 0.9, y1 + 1.05, math.pi / 2, M, C)
    chair("서재_의자2", x1 + 0.9, y1 + 1.95, math.pi / 2, M, C)
    box("서재_모니터1", x1 + 0.05, y1 + 0.8, 0.9, x1 + 0.08, y1 + 1.3, 1.2, M["tv"], C)
    box("서재_모니터2", x1 + 0.05, y1 + 1.7, 0.9, x1 + 0.08, y1 + 2.2, 1.2, M["tv"], C)
    shelf_unit("서재_상부선반", x1, y1 + 0.55, x1 + 0.3, y1 + 2.45, 1.45, H - 0.02, 3, M, C,
               facing="y")
    # 북측 벽 발코니 창 옆 좁은 책장
    shelf_unit("서재_책장", X(108.6) + 1.1, y2 - 0.33, x2, y2, 0, H - 0.02, 6, M, C, facing="x")
    # 동측 벽 소파베드 (게스트용)
    box("서재_소파베드", x2 - 0.9, y1 + 0.9, 0.08, x2 - 0.02, y2 - 0.4, 0.45, M["fabric_gray"], C,
        bevel=0.03)
    box("서재_소파베드_등", x2 - 0.25, y1 + 0.9, 0.45, x2 - 0.02, y2 - 0.4, 0.85, M["fabric_gray"], C,
        bevel=0.05)
    box("서재_러그", x1 + 1.3, y1 + 0.9, 0, x2 - 1.0, y2 - 0.5, 0.012, M["rug"], C)
    # 서재 뒤 발코니(비확장): 수납선반
    CB = coll("14b_서재뒤발코니", coll("05_가구"))
    shelf_unit("발코니_수납선반", X(48) + 0.1, Y(24.5) - 0.5, X(77.5), Y(24.5) - 0.12, -0.05, 1.8, 4,
               M, CB, facing="x", books=False)
...
def build_kids(M):
    for idx, (rx1, rx2, door_px) in enumerate([(263.5, 343.5, 283), (343.5, 424.5, 362)], start=1):
        ...
        # 확장부 창가 책상 (전폭) + 상부 선반
        if mirror:
            # 우수관 박스 (업체 설계도 3·18쪽 판독: 0.60×0.37 m, 관 2개, 창 쪽에 낮은 장)
            box("설비_우수관박스", x2 - 0.60, y1 + 0.37, 0, x2, y1 + 0.74, H, M["paint"], C)
            box("자녀방2_우수관앞장", x2 - 0.60, y1, 0, x2, y1 + 0.37, 0.75, M["cab_white"], C,
                bevel=0.003)
        dx2 = x2 - (0.62 if mirror else 0.05)
        desk(f"자녀방{idx}_책상", x1 + 0.05, y1 + 0.05, dx2, y1 + 0.65, M, C, top="cab_white")
...
def build_entry_misc(M):
    ...
    lx1, lx2 = X(250) + 0.06, X(305) - 0.075
    ly1, ly2 = Y(53.5) + 0.075, Y(5.5) - T_EXT / 2
    box("다용도실_세탁기", lx1 + 0.05, ly2 - 0.7, -0.05, lx1 + 0.75, ly2 - 0.02, 0.85, M["cab_white"], C,
        bevel=0.02)
    box("다용도실_건조기", lx1 + 0.05, ly2 - 0.7, 0.85, lx1 + 0.75, ly2 - 0.02, 1.7, M["cab_white"], C,
        bevel=0.02)
    cyl("다용도실_세탁기문", lx1 + 0.4, ly2 - 0.71, 0.3, 0.3001, 0.2, M["tv"], C)
    box("다용도실_싱크장", lx1 + 0.8, ly2 - 0.6, -0.05, lx1 + 1.35, ly2 - 0.02, 0.85, M["cab_greige"], C,
        bevel=0.003)
    # 보일러 + 분배기: 다용도실 북동쪽 모서리 (업체 설계도 판독 0.33×0.40 m, 위치 ±0.1 m)
    bx1, bx2 = lx2 - 0.34, lx2 - 0.01
    by1, by2 = ly2 - 0.57, ly2 - 0.17
    box("설비_보일러", bx1, by1, 1.25, bx2, by2, 1.95, M["cab_white"], C, bevel=0.01)
    box("설비_보일러_배관", bx1 + 0.08, by1 + 0.1, 0.55, bx2 - 0.08, by1 + 0.14, 1.25, M["steel"], C)
    box("설비_분배기", bx1 + 0.02, by1 + 0.02, 0.15, bx2 - 0.02, by1 + 0.12, 0.55, M["steel"], C)
    box("다용도실_선반", lx1 + 0.02, ly1 + 0.02, 1.8, lx2 - 0.02, ly1 + 0.4, 1.83, M["cab_white"], C)
...
def build_system_ac(M, C):
    """천장 매립형 시스템에어컨 (사용자 요청: 모든 방). 위치는 조명·가구를 피해 배치한 디자인 값."""
    units = [
        ("거실", 6.3, 1.9, 0.95), ("주방식당", 6.85, 7.3, 0.95), ("안방", 3.2, 1.9, 0.95),
        ("서재", 3.5, 7.25, 0.62), ("자녀방1", 9.9, 1.25, 0.62), ("자녀방2", 12.65, 1.25, 0.62),
    ]
    for n, cx, cy, w in units:
        h = w / 2
        box(f"시스템에어컨_{n}", cx - h, cy - h, H - 0.03, cx + h, cy + h, H - 0.001, M["cab_white"], C,
            bevel=0.004)
        g = h * 0.55
        box(f"시스템에어컨_{n}_그릴", cx - g, cy - g, H - 0.032, cx + g, cy + g, H - 0.03,
            M["cab_greige"], C)
...
def build_lighting():
    ...
    rooms = {
        ...
        "서재": pxrect(77.5, 53.5, 162.5, 141.5),
...
CAMERAS = {
    ...
    "06_서재": ((4.75, 5.8, 1.5), (2.3, 8.4, 0.9), 16, None, (1440, 900), False),
...
def build_labels(M):
    ...
    labels = [
        ...
        ("다용도실", P(250, 5.5, 305, 53.5), (0.0, -0.4)),
        ("발코니(비확장)", P(48, 24.5, 162.5, 53.5), (0.0, 0.0)),
    ...
    o.location = (X(18.5), Y(343.5) - 0.38, H + 0.3)
    o.data.materials.append(M["label_sub"])
    o.visible_shadow = False
    C.objects.link(o)
    return font
...
def render_all(quick=False, only=None):
    ...
    for n, (_loc, _tgt, _lens, ortho, (rx, ry), cut) in CAMERAS.items():
        ...
        set_visibility(cut)
        legend = bpy.data.objects.get("라벨_범례")
        if legend is not None:          # 범례는 평면도에만
            legend.hide_render = not ortho
