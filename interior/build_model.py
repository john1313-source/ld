...
def build_architecture(M):
    ...
    keep = [
        ...
        ("유지_대피공간_북", (250, 53.5), (325, 53.5), [D(286, 0.55, head=1.9, swing=1, style="louver")]),
...
def build_kids(M):
    for idx, (rx1, rx2, door_px) in enumerate([(263.5, 343.5, 283), (343.5, 424.5, 362)], start=1):
        ...
        # 확장부 창가 책상 (전폭) + 상부 선반
        if mirror:
            # 우수관 박스 (업체 설계도 18쪽 "우수관 장 사용불가" — 위치·크기는 도면 판독 추정)
            box("설비_우수관박스", x2 - 0.30, y1, 0, x2, y1 + 0.30, H, M["paint"], C)
        dx2 = x2 - (0.35 if mirror else 0.05)
...
def build_entry_misc(M):
    ...
    box("다용도실_싱크장", lx2 - 0.55, ly2 - 0.6, -0.05, lx2 - 0.02, ly2 - 0.02, 0.85, M["cab_greige"], C,
        bevel=0.003)
